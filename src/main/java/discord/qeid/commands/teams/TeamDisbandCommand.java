    ...
    public static LiteralCommandNode<CommandSourceStack> buildSubcommand() {
        return Commands.literal("disband")
            .executes(ctx -> {
                ...
                if (team == null) {
                    player.sendMessage(ConfigUtil.get("team.disband.not-in-team"));
                    return Command.SINGLE_SUCCESS;
                }

                if (!team.getOwner().equals(uuid)) {
                    player.sendMessage(ConfigUtil.get("team.disband.not-owner"));
                    return Command.SINGLE_SUCCESS;
                }
                ...
                if (pendingDisbands.containsKey(uuid)) {
                    ...
                    if (now - started <= 10_000) {
                        ...
                        if (!deleted) {
                            player.sendMessage(ConfigUtil.get("team.disband.failed"));
                            return Command.SINGLE_SUCCESS;
                        }

                        player.sendMessage(ConfigUtil.get("team.disband.success")
                        ...
                        TeamMessengerListener.broadcastExcluding(updatedTeam, uuid,
                            ConfigUtil.get("team.notifications.owner-disbanded")
                                .replace("%player%", player.getName())
                                .replace("%team%", team.getName()));
                        return Command.SINGLE_SUCCESS;
                    } else {
                        pendingDisbands.remove(uuid);
                        player.sendMessage(ConfigUtil.get("team.disband.timeout"));
                        return Command.SINGLE_SUCCESS;
                    }
                }

                pendingDisbands.put(uuid, now);
                player.sendMessage(ConfigUtil.get("team.disband.confirm"));
